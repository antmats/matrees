...
from .tree import info_gain_scorer, gini_scorer, TreeNode
...
class MADTClassifier(ClassifierMixin, BaseEstimator):
    ...
    def _make_tree(self, X, y, M, sample_weight, depth, random_state):
        ...
        if depth >= self.max_depth or self._is_homogeneous(node_value):
            return TreeNode(
                node_value,
        ...
        _split_score, split_feature, split_threshold = max(
            self._get_split_candidates(X, y, M, sample_weight, random_state),
        )
    ...
    def _get_split_candidates(self, X, y, M, sample_weight, random_state):
        n_features = X.shape[1]
        features = np.arange(n_features)
        random_state.shuffle(features)
    ...
    def _is_homogeneous(self, node_value):
